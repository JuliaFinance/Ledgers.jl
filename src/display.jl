...
AT.children(a::Account) = isempty(a.subaccounts) ? Vector{Account}() : a.subaccounts
function AT.printnode(io::IO,a::Account)
    isequal(a,a.parent) ? 
    print(io,a.name) : isempty(a.subaccounts) ? 
    print(io,"$(a.code) $(a.name): ",a.balance) : print(io,"$(a.code) $(a.name)")
end
Base.show(io::IO,a::Account) = isempty(a.subaccounts) ? AT.printnode(io,a) : print_tree(io,a)
Base.show(io::IO,::MIME"text/plain",a::Account) = isempty(a.subaccounts) ? AT.printnode(io,a) : print_tree(io,a)
...
Base.show(io::IO,p::Pair{String,Account}) = print(io,"$(p[1]): $(p[2].name)")
